...
type CustomerStatsRow = { customer_id: string; completed_visits: number };
...
export async function loadReservationsSnapshot(date: string): Promise<FloorSnapshot> {
  ...
  const [assignmentsResult, statsResult] = await Promise.all([
    ...
    customerIds.length
      ? supabase.from('customer_operational_stats').select('customer_id, completed_visits').in('customer_id', customerIds)
      : Promise.resolve({ data: [], error: null }),
  ]);
  ...
  for (const row of (statsResult.data ?? []) as CustomerStatsRow[]) {
    visitsByCustomer.set(String(row.customer_id), Number(row.completed_visits));
  }
  ...
  const tables: DiningTable[] = tableRows.map((row, index) => {
    ...
    return {
      ...
      seats: Number(row.max_capacity),
      minSeats: Number(row.min_capacity),
      areaId: String(row.area_id),
      area: asArea(relation?.name ? String(relation.name) : null),
      x: 0,
      y: index,
      w: 0,
      h: 0,
  ...
  const reservations: AdminReservation[] = reservationRows.map((row) => {
    ...
    return {
      ...
      visits: customerId ? (visitsByCustomer.get(customerId) ?? 0) : 0,
    };
  });
