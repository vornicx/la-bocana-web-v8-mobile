...
export function PublicFooter({ locale = 'es' }: { locale?: PublicLocale }) {
  ...
  return (
    <footer className="public-footer">
      ...
      <nav aria-label={copy.legal}><Link href={legalPaths.legal}>{copy.legalNotice}</Link><Link href={legalPaths.privacy}>{copy.privacy}</Link><Link href={legalPaths.cookies}>{copy.cookies}</Link><Link href={legalPaths.terms}>{copy.terms}</Link><a href="https://www.instagram.com/restaurantelabocana" target="_blank" rel="noreferrer">Instagram</a><CookieSettingsButton locale={locale} /></nav>
