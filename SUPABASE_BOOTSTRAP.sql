...
create index if not exists rate_limit_lookup_idx on public.rate_limit_events(key_hash, action, created_at desc);
...
begin
  ...
  select count(*) into v_count from public.rate_limit_events where key_hash=p_key_hash and action=p_action and created_at >= now()-make_interval(secs=>p_window_seconds);
  if v_count >= p_limit then return false; end if;
