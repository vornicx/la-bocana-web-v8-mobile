...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  return <div className="admin-page"><div className="admin-page-head"><div><span className="admin-kicker">Planificación · datos reales</span><h1>Calendario</h1><p>Capacidad prevista y presión por servicio durante la semana.</p></div><div className="admin-head-actions"><Link className="admin-secondary" href={`/admin/calendario?week=${calendar.previous}`} aria-label="Semana anterior">‹</Link><strong>{dateLabel(calendar.start, { day: 'numeric', month: 'short' })}–{dateLabel(calendar.end, { day: 'numeric', month: 'short', year: 'numeric' })}</strong><Link className="admin-secondary" href={`/admin/calendario?week=${calendar.next}`} aria-label="Semana siguiente">›</Link></div></div>
    <div className="calendar-week real-calendar-week">{calendar.days.map((day) => <div key={day.date} className={`calendar-day ${day.active ? 'active' : ''}`}><header><span>{dateLabel(day.date, { weekday: 'short' })}</span><strong>{dateLabel(day.date, { day: 'numeric' })}</strong></header><div className="calendar-canvas">{day.closed ? <div className="closed-card">Cerrado</div> : <div className="calendar-real-content"><div className="calendar-day-total"><strong>{day.covers} pax</strong><small>{day.reservations} reservas</small></div>{day.services.map((service) => <div className="calendar-service" key={service.id}><div><span>{service.name}</span><strong>{service.covers} pax</strong></div><i><b style={{ width: `${service.occupancy}%` }}/></i><small>{service.occupancy}% de ocupación</small></div>)}</div>}</div></div>)}</div>
  </div>;
}
