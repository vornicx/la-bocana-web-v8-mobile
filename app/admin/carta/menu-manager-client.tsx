...
export function MenuManagerClient({ initialCategories, canEdit }: { initialCategories: ManagedMenuCategory[]; canEdit: boolean }) {
  ...
  function changeType(next: 'food' | 'wine') {
    setType(next);
  ...
  return <div className="admin-page menu-manager-page">
    ...
    <div className="menu-manager-command"><div className="segmented"><button className={type === 'food' ? 'active' : ''} onClick={() => changeType('food')}>Carta</button><button className={type === 'wine' ? 'active' : ''} onClick={() => changeType('wine')}>Vinos</button></div><div><span>{visible.length} categorías</span><span>{itemCount} referencias</span><span className={photos < itemCount ? 'attention' : ''}>{photos} con fotografía oficial</span></div></div>
    {message && <div className="admin-feedback" role="status">{message}</div>}
    <div className="menu-manager-list">{visible.map((category, index) => {
      const isOpen = openCategoryId === category.id;
      return <details className="menu-manager-category" key={category.id} open={isOpen} onToggle={(event) => {
        const nextOpen = event.currentTarget.open;
        if (nextOpen) setOpenCategoryId(category.id);
        else if (openCategoryId === category.id) setOpenCategoryId(null);
      }}>
        <summary><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{category.nameEs}</strong><small>{category.items.length} referencias · {category.items.filter((item) => item.active).length} publicadas</small></div><i className={category.active ? 'published' : ''}>{category.active ? 'Publicada' : 'Oculta'}</i></summary>
        {isOpen && <div className="menu-category-editor"><div className="menu-category-fields"><label><span>Nombre ES</span><input disabled={!canEdit} value={category.nameEs} onChange={(event) => updateCategory(category.id, { nameEs: event.target.value })}/></label><label><span>Nombre EN</span><input disabled={!canEdit} value={category.nameEn ?? ''} onChange={(event) => updateCategory(category.id, { nameEn: event.target.value || null })}/></label><label><span>Descriptor ES</span><input disabled={!canEdit} value={category.eyebrowEs ?? ''} onChange={(event) => updateCategory(category.id, { eyebrowEs: event.target.value || null })}/></label><label><span>Descriptor EN</span><input disabled={!canEdit} value={category.eyebrowEn ?? ''} onChange={(event) => updateCategory(category.id, { eyebrowEn: event.target.value || null })}/></label><label className="toggle-label"><input type="checkbox" disabled={!canEdit} checked={category.active} onChange={(event) => updateCategory(category.id, { active: event.target.checked })}/><span>Publicar categoría</span></label><button className="admin-secondary" disabled={!canEdit || busy === category.id} onClick={() => save('category', category)}>Guardar categoría</button></div>
